...
async function loadSiswaDanNilai() {
  ...
  siswa.forEach((item, index) => {
    ...
    // Jika nilai lama null/undefined/kosong, biarkan value input-nya "" (string kosong)
...
saveBtn.addEventListener('click', async () => {
  ...
  for (const [index, item] of siswaData.entries()) {
    ...
    // Gabungkan data lama dengan data baru yang baru diinput
    const merged = {
      user_id: user.id,
      ...
      // Timpa jenis nilai yang sedang dipilih saat ini
      [jenisSelect.value]: nilaiInput
    }

    // Hitung Rata-Rata Nilai Akhir Otomatis (Hanya merata-ratakan nilai yang TERISI)
